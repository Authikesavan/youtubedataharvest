...
p1 = Image.open('G:\\youtube.proj\\youtube.main.webp\\')
p2 = Image.open('G:\\youtube.proj\\youtube.use1.jpg\\')
p3 = Image.open('G:\\youtube.proj\\data-migration.webp\\')
...
choice  = st.sidebar.radio('navigate',["MAIN","DATA SCRAPPING","MIGRATION","VISUALIZATION"])


if choice == "MAIN":
   st.image(p1)
   st.markdown("ABOUT YOUTUBE AND ITS DATA")
   st.write("Youtube is one of the largest video streaming application with  numbers of users through out the world.",
            "Its data serves in a greater considering the user count.",
            "Here we harvest data from youtube and tries to make some sense out of those data.",
            "Youtube Api v3 plays the key role in scrapping data from youtube.")
   


elif choice == "DATA SCRAPPING":
     
     st.header("DATA HARVEST")
     st.image(p2)
    ...
    # Define Session state to Get data button
     ...
     if Get_data or st.session_state.Get_state:
        ...
        upload = col.replace_one({'_id': channel_id}, final_output_data, upsert=True)
    ...
    if  Migrate or st.session_state_migrate_sql:
        ...
        result = col.find_one({"Channel_Name": document_name})
        client.close()
        ...
        mycursor.execute("CREATE DATABASE IF NOT EXISTS yt_db")
    ...
    st.header(':red[Channel Data Analysis ]')
...
# Q8
         ...
         cursor.execute("SELECT channel.Channel_Name, video.Video_Name, video.Published_date FROM channel JOIN playlist ON channel.Channel_Id = playlist.Channel_Id JOIN video ON playlist.Playlist_Id = video.Playlist_Id" 
                        "FROM Published_date =2022"
                        "WHERE EXTRACT YEAR")
